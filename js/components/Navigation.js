/**
 * Global Navigation System & Mobile Ergonomic Dock
...
export class Navigation {
  ...
  renderHeader() {
    ...
    this.navEl.innerHTML = `
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-2">
        <!-- Logo & System Status -->
        <a href="#hero" class="flex items-center gap-2.5 sm:gap-3 group shrink-0">
          ...
          <div class="truncate">
            <div class="font-mono font-bold text-xs tracking-wider text-text-primary group-hover:text-cyan transition-colors uppercase truncate">
              KHALID ABDULLAH
            </div>
            <div class="flex items-center gap-1.5 text-[9px] sm:text-[10px] font-mono text-text-muted">
              <span class="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
              <span class="hidden xs:inline">DIGITAL LAB</span>
              <span class="text-border hidden xs:inline">|</span>
        ...
        <nav class="hidden md:flex items-center gap-1 lg:gap-2">
          ...
          <a href="admin.html" class="px-2.5 py-1.5 rounded-lg text-xs font-mono text-cyan bg-cyan/10 hover:bg-cyan/25 border border-cyan/40 transition-all flex items-center gap-1 font-bold" title="Admin Studio (CMS)">
            <span>⚙️</span> Studio
          </a>
        </nav>
        ...
        <div class="flex items-center gap-1.5 sm:gap-2 shrink-0">
          <!-- Command Palette Trigger (Desktop only, mobile has it in dock) -->
          <button id="btn-cmd-k" class="hidden md:flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-surface-elevated border border-border text-xs font-mono text-text-secondary hover:text-cyan hover:border-cyan/50 transition-all cursor-pointer" title="Quick Search (Cmd+K)">
            ...
            <span>Search</span>
            <kbd class="px-1.5 py-0.5 text-[10px] rounded bg-surface border border-border text-text-muted">⌘K</kbd>
          </button>

          <!-- Terminal Trigger (Desktop only) -->
          <button id="btn-open-terminal" class="hidden md:flex p-2 rounded-lg bg-surface-elevated border border-border text-text-secondary hover:text-emerald-400 hover:border-emerald-400/50 transition-all cursor-pointer" title="Open Interactive CLI Terminal (~)">
          ...
          <!-- Studio Link for Mobile Header -->
          <a href="admin.html" class="md:hidden p-1.5 px-2.5 rounded-lg bg-cyan/15 border border-cyan/40 text-cyan text-xs font-mono font-bold transition-all">
            <span>⚙️ Studio</span>
          </a>

          <!-- Contact Dispatch Button (Desktop only) -->
          <a href="#about" class="hidden md:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-cyan/15 hover:bg-cyan/25 border border-cyan/40 text-cyan text-xs font-mono font-semibold transition-all">
            <span>Connect</span>
            <span>→</span>
          </a>
        </div>
  ...
  renderMobileDock() {
    ...
    this.mobileDockEl.innerHTML = `
      <div class="fixed bottom-3 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-sm px-2 py-1.5 rounded-2xl bg-surface/95 backdrop-blur-xl border border-border shadow-2xl flex items-center justify-between md:hidden">
        <a href="#hero" class="mobile-dock-btn p-1 rounded-xl text-text-secondary hover:text-cyan flex flex-col items-center gap-0.5 transition-colors" data-section="hero">
          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
          <span class="text-[9px] font-mono leading-none">Home</span>
        </a>
        <a href="#projects" class="mobile-dock-btn p-1 rounded-xl text-text-secondary hover:text-cyan flex flex-col items-center gap-0.5 transition-colors" data-section="projects">
          <span class="text-xs leading-none">⚡</span>
          <span class="text-[9px] font-mono leading-none">Projects</span>
        </a>
        <a href="#posts" class="mobile-dock-btn p-1 rounded-xl text-text-secondary hover:text-cyan flex flex-col items-center gap-0.5 transition-colors" data-section="posts">
          <span class="text-xs leading-none">📝</span>
          <span class="text-[9px] font-mono leading-none">Posts</span>
        </a>
        <a href="#tools" class="mobile-dock-btn p-1 rounded-xl text-text-secondary hover:text-cyan flex flex-col items-center gap-0.5 transition-colors" data-section="tools">
          <span class="text-xs leading-none">🛠️</span>
          <span class="text-[9px] font-mono leading-none">Tools</span>
        </a>
        <a href="admin.html" class="mobile-dock-btn p-1 rounded-xl text-cyan hover:text-cyan-glow flex flex-col items-center gap-0.5 transition-colors">
          <span class="text-xs leading-none">⚙️</span>
          <span class="text-[9px] font-mono leading-none font-bold">Studio</span>
        </a>
        <button id="mobile-cmd-btn" class="p-1 px-2 rounded-xl text-text-secondary bg-surface-elevated border border-border flex flex-col items-center gap-0.5 cursor-pointer">
          <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
          <span class="text-[9px] font-mono leading-none">Search</span>
        </button>
      </div>
    `;
  }
