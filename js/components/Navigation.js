  ...
  renderMobileDock() {
    ...
    this.mobileDockEl.innerHTML = `
      <div class="fixed bottom-3 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-sm px-2 py-1.5 rounded-2xl bg-surface/95 backdrop-blur-xl border border-border shadow-2xl flex items-center justify-between md:hidden">
        ...
        <a href="#build-log" class="mobile-dock-btn p-1 rounded-xl text-text-secondary hover:text-cyan flex flex-col items-center gap-0.5 transition-colors" data-section="build-log">
          <span class="text-xs leading-none">⏱️</span>
          <span class="text-[9px] font-mono leading-none">Log</span>
        </a>
        <button id="mobile-cmd-btn" class="p-1 px-2 rounded-xl text-cyan bg-cyan/15 flex flex-col items-center gap-0.5 cursor-pointer">
          <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
          <span class="text-[9px] font-mono leading-none font-bold">Search</span>
        </button>
      </div>
    `;
  }
