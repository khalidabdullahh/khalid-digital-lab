/**
 * Projects Showcase Component (Featured, Supporting, & Live GitHub Repositories)
...
export class ProjectsSection {
    ...
    this.container.innerHTML = `
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div class="flex flex-wrap items-end justify-between gap-6 mb-12">
          ...
          <!-- Live GitHub Auto-Sync Controller -->
          <div class="flex flex-wrap items-center gap-3">
            <button id="btn-sync-github" class="flex items-center gap-2 px-4 py-2 rounded-xl bg-surface-elevated hover:bg-border border border-border hover:border-cyan/40 text-xs font-mono font-bold text-cyan transition-all shadow-sm cursor-pointer" title="Fetch latest updates from GitHub">
              ...
              <span id="sync-label">Sync GitHub Repos</span>
            </button>
        ...
        <div class="space-y-8 mb-12">
          ${featuredProjects.map(proj => {
            const repo = githubService.findRepoForUrl(proj.githubUrl);
            return `
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                  ...
                  <div class="lg:col-span-7 space-y-5">
                    <div class="flex flex-wrap items-center gap-2.5">
                      ...
                      <!-- Live GitHub Telemetry Badge -->
                      ${repo ? `
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-mono bg-surface-elevated border border-border text-text-secondary">
                          <span class="text-amber-400">★</span>
                          <span>${repo.stars}</span>
                          <span class="text-border">|</span>
                          <span class="text-cyan">${githubService.getTimeAgo(repo.pushedAt)}</span>
                        </span>
                      ` : ""}
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-3 pt-4">
                      ${proj.liveUrl && proj.liveUrl !== proj.githubUrl ? `
                        <a href="${proj.liveUrl}" target="_blank" rel="noopener noreferrer" class="px-5 py-2.5 rounded-xl bg-cyan hover:bg-cyan-glow text-black text-xs font-mono font-bold transition-all shadow-lg flex items-center gap-2">
                          <span>${proj.category.includes("Game") ? "Play Game Live ↗" : "Launch Live Application ↗"}</span>
                        </a>
                      ` : ""}
              ...
              <div class="group relative p-6 rounded-2xl bg-surface border border-border hover:border-cyan/50 hover:bg-surface-elevated/70 transition-all duration-300 flex flex-col justify-between shadow-xl">
                <div>
                  ...
                  <div class="flex flex-wrap items-center gap-1.5 mt-4">
                    ...
                    ${repo ? `
                      <span class="px-2 py-0.5 rounded bg-surface-elevated border border-border/60 text-[10px] font-mono text-amber-400">
                        ★ ${repo.stars}
                      </span>
                    ` : ""}
                  </div>
                ...
                <div class="mt-6 pt-4 border-t border-border/70 flex items-center justify-between">
                  ...
                  <a href="${proj.githubUrl}" target="_blank" rel="noopener noreferrer" class="text-text-muted hover:text-text-primary transition-colors flex items-center gap-1 text-xs font-mono">
                    ...
                    <span>${repo ? githubService.getTimeAgo(repo.pushedAt) : "GitHub"}</span>
                  </a>
                </div>
              </div>
            `;
        ...
        <!-- 3. Dynamic GitHub Repositories Feed (Auto-Updates from GitHub) -->
        <div class="p-6 sm:p-8 rounded-3xl bg-surface-elevated/40 border border-border shadow-xl">
          <div class="flex flex-wrap items-center justify-between gap-4 mb-6 pb-4 border-b border-border">
            <div class="flex items-center gap-3">
              <div class="p-2 rounded-xl bg-surface border border-border text-cyan">
              ...
              <div>
                <div class="flex items-center gap-2">
                  <span class="text-xs font-mono font-bold text-cyan uppercase tracking-wider">LIVE GITHUB FEED</span>
                  <span class="px-2 py-0.5 rounded-full text-[10px] font-mono bg-emerald-500/15 text-emerald-300 border border-emerald-500/30">● Real-Time Sync</span>
                </div>
                <h4 class="text-base font-bold text-text-primary mt-0.5">All Public Repositories (@khalidabdullahh)</h4>
              </div>
            </div>

            <a href="https://github.com/khalidabdullahh?tab=repositories" target="_blank" rel="noopener noreferrer" class="text-xs font-mono text-cyan hover:underline flex items-center gap-1.5">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ${liveRepos.map(repo => `
              <div class="p-4 rounded-xl bg-surface border border-border hover:border-cyan/40 transition-all flex flex-col justify-between shadow-sm">
                <div>
                  <div class="flex items-center justify-between mb-2">
                    <a href="${repo.htmlUrl}" target="_blank" rel="noopener noreferrer" class="text-sm font-bold text-text-primary hover:text-cyan transition-colors font-mono line-clamp-1">
                      ${repo.name}
                    </a>
                    <span class="text-[10px] font-mono text-text-muted">${githubService.getTimeAgo(repo.pushedAt)}</span>
                  </div>
                ...
                <div class="mt-4 pt-3 border-t border-border/60 flex items-center justify-between text-xs font-mono">
                  <div class="flex items-center gap-3">
                    <span class="flex items-center gap-1.5 text-text-secondary text-[11px]">
                    ...
                    <span class="text-text-muted text-[11px] flex items-center gap-0.5">
                      <span class="text-amber-400">★</span>
                      <span>${repo.stars}</span>
                    </span>
                  </div>

                  <a href="${repo.htmlUrl}" target="_blank" rel="noopener noreferrer" class="text-cyan hover:underline text-[11px] font-bold">
                    Code ↗
                  </a>
                </div>
              </div>
  ...
  bindEvents() {
    ...
    syncBtn?.addEventListener("click", async () => {
      ...
      setTimeout(() => {
        if (syncLabel) syncLabel.textContent = "Sync GitHub Repos";
      }, 2000);
    });
