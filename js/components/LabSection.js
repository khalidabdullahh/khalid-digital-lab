    ...
    this.container.innerHTML = `
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div class="flex flex-wrap items-end justify-between gap-6 mb-12">
          <div>
            <div class="flex items-center gap-2 text-xs font-mono font-bold tracking-widest text-cyan uppercase mb-2">
              <span>🧪</span>
              <span>THE LAB // EXPERIMENTAL DASHBOARD</span>
            </div>
            <h2 class="text-3xl sm:text-4xl font-extrabold text-text-primary tracking-tight">
              Rigorous Research & Scientific Inquiries
            </h2>
            <p class="text-base text-text-secondary mt-2 max-w-2xl">
              Every system starts with a hypothesis. Explore ongoing research experiments across quantitative finance, machine learning models, and high-performance software systems.
            </p>
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ${filtered.map(exp => `
            <div class="experiment-card group relative p-6 rounded-2xl bg-surface border border-border hover:border-cyan/50 hover:bg-surface-elevated/70 transition-all duration-300 flex flex-col justify-between shadow-xl cursor-pointer" data-exp-id="${exp.id}">
              <div>
                <!-- Status & Category -->
                <div class="flex items-center justify-between gap-2 mb-4">
                  <span class="px-2.5 py-0.5 rounded-full text-[10px] font-mono font-semibold bg-surface-elevated border border-border text-text-secondary">
                    ${exp.category}
                  </span>
                ...
                <!-- Title -->
                <h3 class="text-lg font-bold text-text-primary group-hover:text-cyan transition-colors tracking-tight leading-snug">
                  ${exp.title}
                </h3>

                <!-- Research Question Box -->
                <div class="mt-4 p-3 rounded-xl bg-surface-elevated/60 border border-border/80">
                  <div class="text-[10px] font-mono font-semibold text-cyan uppercase mb-1">RESEARCH QUESTION</div>
              ...
              <div class="mt-6 pt-4 border-t border-border/70 flex items-center justify-between">
                ...
                <span class="text-xs font-mono font-semibold text-cyan group-hover:translate-x-1 transition-transform flex items-center gap-1">
                  <span>Examine</span>
                  <span>→</span>
                </span>
              </div>
  ...
  openModal(exp) {
    ...
    modalContent.innerHTML = `
      ...
      <div class="flex items-start justify-between gap-4 pb-6 border-b border-border">
        <div>
          <div class="flex items-center gap-2 mb-2">
            ...
            <span class="px-2.5 py-0.5 rounded-full text-xs font-mono font-semibold ${
              exp.statusColor === "emerald" ? "bg-emerald-500/15 text-emerald-300 border border-emerald-500/30" :
              exp.statusColor === "amber" ? "bg-amber-500/15 text-amber-300 border border-amber-500/30" :
              "bg-blue-500/15 text-blue-300 border border-blue-500/30"
            }">
            ...
            <span class="text-xs font-mono text-text-muted">Updated: ${exp.lastUpdated}</span>
          </div>
      ...
      <div class="space-y-6 py-6 text-sm">
        ...
        <div class="space-y-3">
          <div class="p-4 rounded-xl bg-cyan/10 border border-cyan/30">
          ...
          <div class="p-4 rounded-xl bg-surface-elevated/70 border border-border">
            <div class="text-xs font-mono font-bold text-amber-400 uppercase mb-1">🔬 Scientific Hypothesis</div>
        ...
        ${exp.mathFormula ? `
          <div class="p-4 rounded-xl bg-[#050810] border border-border font-mono text-center overflow-x-auto text-cyan">
            <div class="text-[10px] text-text-muted uppercase mb-1">Mathematical Formulation</div>
        ...
        ${exp.codeSnippet ? `
          <div>
            <div class="flex items-center justify-between mb-2">
              <span class="text-xs font-mono font-bold text-text-muted uppercase">Algorithm & Code Snippet</span>
              <button id="btn-copy-code" class="px-2.5 py-1 text-[11px] font-mono rounded bg-surface-elevated hover:bg-border text-text-secondary hover:text-cyan transition-all flex items-center gap-1">
                <span>Copy Code</span>
              </button>
            </div>
            <pre class="p-4 rounded-xl bg-[#050810] border border-border text-xs font-mono text-text-secondary overflow-x-auto leading-relaxed max-h-48"><code>${exp.codeSnippet}</code></pre>
          </div>
        ` : ""}
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="p-4 rounded-xl bg-surface-elevated/40 border border-border">
            <div class="text-xs font-mono font-bold text-emerald-400 uppercase mb-2">📊 Empirical Results</div>
          ...
          <div class="p-4 rounded-xl bg-surface-elevated/40 border border-border">
            <div class="text-xs font-mono font-bold text-violet-400 uppercase mb-2">💡 Key Learnings</div>
            <ul class="space-y-1.5">
      ...
      <div class="flex flex-wrap items-center justify-between gap-3 pt-6 border-t border-border">
        <div class="flex items-center gap-2">
          ${exp.interactiveDemoAvailable ? `
            <a href="#tools" class="modal-jump-link px-4 py-2 rounded-xl bg-cyan hover:bg-cyan-glow text-black text-xs font-mono font-bold transition-all flex items-center gap-1.5 shadow-lg">
    ...
    document.querySelector(".modal-jump-link")?.addEventListener("click", () => {
      this.closeModal();
    });
